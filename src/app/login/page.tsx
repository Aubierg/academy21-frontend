...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch (err: unknown) { setError(err instanceof Error ? err.message : 'Identifiants incorrects'); }
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '68px', display: 'flex', background: '#f7f8fa' }}>
      <div style={{ width: '420px', flexShrink: 0, background: '#C8102E', padding: '80px 60px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }} className="auth-panel">
        ...
        <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '16px', lineHeight: 1.8, marginBottom: '40px' }}>Votre espace personnel pour accéder à vos formations et suivre votre progression.</p>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ width: '100%', maxWidth: '420px', animation: 'fadeUp 0.5s ease' }}>
          ...
          <p style={{ color: '#999', fontSize: '15px', marginBottom: '36px' }}>Bienvenue ! Connectez-vous à votre espace membre.</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading} className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', fontSize: '14px', padding: '15px' }}>
              {loading ? <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><span style={{ width: '16px', height: '16px', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.6s linear infinite' }} />Connexion...</span> : 'Se connecter'}
            </button>
          </form>
          <div className="divider" />
          <p style={{ textAlign: 'center', color: '#999', fontSize: '14px' }}>
            Pas encore de compte ?{' '}<Link href="/register" style={{ color: '#C8102E', fontWeight: 700 }}>S&apos;inscrire gratuitement</Link>
          </p>
        </div>
      </div>
      <style>{`
